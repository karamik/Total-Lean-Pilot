...
| **ChainID** | `888888` | Официальный ID Lean Pilot (genesis.json) |
...
> **Примечание:** Все поля `accrued*` дублируются ончейн через публичные геттеры контракта `FeeSplitter`. Для проверки баланса конкретного адреса используйте `FeeSplitter.pendingRewards(address)` или `FeeSplitter.getShare(address)` — значения идентичны RPC-ответу.
...
const CONFIG = {
    ...
    CHAIN_ID: 888888,            // Lean Pilot ChainID (genesis.json)
...
async function onNewBlock(blockNumber) {
    ...
    try {
        ...
        // В PoA сеть gasPrice не нужен — вся комиссия через value
        const tx = await wallet.sendTransaction({
            to: opp.targetContract,
            value: ethers.parseEther("0.01"), // Фиксированная комиссия пилота ($0.01)
            gasLimit: CONFIG.GAS_LIMIT,
            chainId: CONFIG.CHAIN_ID,         // 888888 — Lean Pilot
            // gasPrice опущен — PoA игнорирует его
        });
...
console.log(`ChainID: ${CONFIG.CHAIN_ID}`);
...
async def on_block(block_number):
    ...
    # Send tx
    # В PoA сети gasPrice = 0 — вся комиссия через value
    # Это экономит ~50-100ms на запросе eth_gasPrice
    tx = {
        'to': FEE_SPLITTER,
        'value': w3.to_wei(0.01, 'ether'),  # Фиксированная комиссия пилота ($0.01)
        'gas': GAS_LIMIT,
        'gasPrice': 0,                       # PoA: gasPrice не используется
        'nonce': w3.eth.get_transaction_count(account.address),
        'chainId': CHAIN_ID                  # 888888 — Lean Pilot
    }
...
async def main():
    print(f"=== TOTAL Lean Pilot Arbitrage Bot ===")
...
В Lean Pilot используется **Clique PoA** с фиксированной комиссией:

- **`gasPrice` должен быть `0`** — нода не использует его для приоритизации
- **Вся комиссия передаётся через `value`** — 0.01 ETH/TIA на адрес FeeSplitter
- **Не запрашивайте `eth_gasPrice`** — экономит 50-100ms на каждой транзакции

❌ **Не делайте:**
- `w3.eth.gas_price` или `provider.getFeeData()` — бесполезно в PoA
- `gasPrice > 0` — нода проигнорирует, но лишние байты в транзакции
- Создавать новый HTTP connection на каждый запрос

✅ **Делайте:**
- `gasPrice: 0` (Python) или опускайте поле (JS — ethers сам ставит 0)
